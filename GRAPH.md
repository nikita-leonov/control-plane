...
**Author is never judge** — structurally, rather than as an instruction in a
prompt that can fall out of context. The Builder cannot talk its way past
`./verify`, because there is no edge that accepts an argument.
